...
<?php 
// Create an XML element for each product in the shopping cart
$shoppingCart = new SimpleXMLElement('<shoppingCart></shoppingCart>');

$result = mysqli_query($conn, "SELECT k.id_keranjang as keranjang, k.kode_produk as kd, k.nama_produk as nama, k.qty as jml, p.image as gambar, p.harga as hrg FROM keranjang k join produk p on k.kode_produk=p.kode_produk WHERE kode_customer = '$kode_cs'");
$no = 1;
$hasil = 0;
while ($row = mysqli_fetch_assoc($result)) {
    $product = $shoppingCart->addChild('product');
    $product->addChild('no', $no);
    $product->addChild('image', $row['gambar']);
    $product->addChild('nama', $row['nama']);
    $product->addChild('harga', $row['hrg']);
    $product->addChild('qty', $row['jml']);
    $product->addChild('subtotal', $row['hrg'] * $row['jml']);
    $sub = $row['hrg'] * $row['jml'];
    $hasil += $sub;
    $no++;
}
...
$xmlContent = $shoppingCart->asXML();

// Save the XML content to a file
file_put_contents('keranjang.xml', $xmlContent);
